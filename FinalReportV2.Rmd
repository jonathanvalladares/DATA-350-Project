...
median_bmi <- clean_data_ridge |> 
  group_by(Age_group, Gender) |>
  summarize(median_BMI = median(BMI, na.rm = TRUE), .groups = "drop") |>
  mutate(label = paste0("(", round(median_BMI, 1), ", ", Gender, ")"))
...
ggplot(clean_data_ridge, aes(x = BMI, y = Age_group, fill = Gender)) +
  ...
  geom_segment( 
    ...
    aes( x = median_BMI, 
         xend = median_BMI, 
         y = as.numeric(Age_group) + 0.05, 
         yend = as.numeric(Age_group) - 0.05 ), 
    color = "black", 
    size = 1.1, 
    show.legend = FALSE ) + 
  geom_text( 
    data = median_bmi |> 
      mutate(
        y_offset = ifelse(Gender == "female",
                          as.numeric(Age_group) - 0.27,
                          as.numeric(Age_group) - 0.15)),
    aes( x = median_BMI, y = y_offset, label = label ), 
    color = "black", 
    size = 3.2, 
    vjust = 0, 
    show.legend = FALSE ) +
...
ggplot(nhanes_sub_labels, aes(x = Gender, y = count, fill = Used)) +
